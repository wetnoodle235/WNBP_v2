# ──────────────────────────────────────────────────────────────────────
# V5.0 Backend — NCAAB Normalized-Curated PyArrow Schemas (Consolidated)
# ──────────────────────────────────────────────────────────────────────
#
...
#
# NCAAB uses date-based games (no week partitioning).
...
schema_teams = pa.schema([
    ...
    _f("name",            pa.string(), "Short team name (e.g. Jayhawks)",            nullable=False),
    _f("abbreviation",    pa.string(), "Team abbreviation (e.g. KU)",                nullable=False),
...
schema_games = pa.schema([
    ...
    # Half scores
    _f("home_h1",         pa.int32(),  "Home 1st half score"),
    _f("home_h2",         pa.int32(),  "Home 2nd half score"),
    _f("away_h1",         pa.int32(),  "Away 1st half score"),
    _f("away_h2",         pa.int32(),  "Away 2nd half score"),
    # Metadata
...
schema_plays = pa.schema([
    ...
    _f("season",        pa.int32(),  "Season start year",                            nullable=False),
    _f("date",          pa.string(), "Game date (ISO-8601)"),
    _f("period",        pa.int32(),  "Game period / half (1, 2, OT1, …)"),
...
schema_bracket = pa.schema([
    ...
    _f("region",         pa.string(), "Tournament region (e.g. East, West, South, Midwest)"),
...
NCAAB_SCHEMAS: dict[str, pa.Schema] = {
...
# to the flat entity directory name under normalized_curated/ncaab/.
# Types mapping to None are intentionally skipped (non-entity artefacts).
NCAAB_TYPE_TO_ENTITY: dict[str, str | None] = {
...
NCAAB_ENTITY_ALLOWLIST: set[str] = {
    "conferences",
    "teams",
    "players",
    "games",
...
NCAAB_STATIC_ENTITIES: set[str] = {"teams", "conferences", "venues"}
